...
| 記法 | 意味 |
|------|------|
| `---`...`---` | フロントマター（タイトル・サブタイトル・作成者・日付）。**丸ごと省略可能**（下記参照） |
| `# タイトル` / `## タイトル` | スライド区切り＆タイトル（H1・H2どちらでも区切りになる） |
| `### 見出し`〜`###### 見出し` | スライドは区切らず、記号を外した小見出し行として本文に含める |
| `- テキスト` | 箇条書き（`・` に変換） |
| `> テキスト` | 引用（quoteフィールドとして左アクセント線付きの帯で表示） |
| `![説明](パス)` | 画像挿入。複数行書くと横並びで挿入され、高さが自動で揃う。パスはMarkdownファイル自身からの相対パス |
| `<!-- summary -->` | まとめスライドとしてマーク（見出し行末に書く） |
| ` ```コードブロック``` ` | 等幅フォント（Courier New）のテキストボックスとして表示される。ディレクトリツリー等のファイル構成表示向け |
| `\|表\|の行\|` | 本物のPowerPoint表（ネイティブテーブル）に変換される |

表・コードブロックは、本文や画像など他の要素と重ならないよう、内容量（文字数・行数）から高さを概算して縦に積み上げて配置する。ただし1枚のスライドに本文＋表＋コードブロックのように要素を詰め込みすぎると、概算しても収まりきらずスライド下端をはみ出すことがある。その場合は表・文字溢れ全般の基本方針どおり、内容を絞る・スライドを分けるなど手動での調整が必要（自動での文章化・省略は行わない）。

### フロントマターを省略する（既存のREADME.md等をそのまま使う）

フロントマターは丸ごと省略できる。`title` を省略した場合、本文の最初の `# 見出し`（H1）、それも無ければ最初の `## 見出し`（H2）を自動でタイトルに使う。`subtitle`/`author`/`date` は元々省略時に空欄になるだけなので、フロントマターを一切書かなくても動作する。

これにより、README.mdなど既存のMarkdownファイルをコピーせずそのまま `python3 make_pptx.py README.md` のように渡しても、タイトル付きのスライドが生成できる（ただし表・コードブロックが多いファイルは、1枚のスライドに情報が詰まりすぎて前述のはみ出しが起きやすいので、内容を絞り込んだ専用ファイルを別途作るのが望ましい）。
...
Markdownの表（`| a | b |`）は本物のPowerPoint表（ネイティブテーブル）に変換される。一時期、表を箇条書きに機械変換する機能を試したが、行の意味関係が失われて逆に読みにくくなったため撤回し、代わりにPowerPointの表オブジェクトとしてそのまま生成する方式にした（Python版・VBA版とも対応）。

同様にコードブロック（```` ``` ````）も省略せず、等幅フォント（Courier New）のテキストボックスとして表示される（ディレクトリツリー等のファイル構成表示向け）。

本文・表・コードブロック・画像は、生成時に文字量から高さを概算して縦に重ならないよう積み上げて配置する。ただし、これは概算にすぎないため、**1枚のスライドに要素を詰め込みすぎるとページの下端をはみ出すことがある**。その場合は自動での文章化・省略は行わないので、**内容を絞り込んで手動調整するのが基本方針**。PowerPointのオートフィット（`autofit`、テキストボックスに収まらない場合にPowerPoint側でフォントサイズを縮小する機能）は保険として有効にしてあるが、これは「読めるサイズを保ったまま収める」ことまでは保証しない。生の技術文書やREADMEを丸ごと貼り付けるのではなく、スライドごとに3〜5個程度の簡潔な箇条書きに要約してから流し込むと、見やすい資料になる。
...
3. 対応する記法は `make_pptx.py` のMarkdown版とほぼ同じ（フロントマターの `title`/`subtitle`/`author`/`date`、`title` 省略時の見出しからの自動取得、`##` 見出し、`-`/`*` 箇条書き、`<!-- summary -->`、`|表|の行|`（本物のPowerPoint表に変換）、```` ```コードブロック``` ````（等幅フォントのテキストボックスに変換））
...
**開発メモ（表・コードブロック対応時に踏んだVBAの罠）**:
- `Shapes.AddTable` で生成した `Table`/`Cell` オブジェクトを扱う自作関数（`AddTableShape`）の引数に `Collection` 型のパラメータを（`ByVal` を付けずに）宣言すると、`For Each x In collection`（`x` は `Variant`）で得た要素をそのまま渡した際に「**コンパイルエラーです。：ByRef 引数の型が一致しません。**」となった。VBAは既定でByRef渡しのため、呼び出し側の実引数が宣言型と完全一致（`Variant` ではなく厳密に `Collection`）していないと弾かれる。`ByVal tbl As Collection` のように明示的に`ByVal`を付けることで解消（オブジェクト引数はByValでも参照そのものは渡るため、関数内で中身を書き換える分には支障ない）
- `If ... Then` ブロックや `For ... Next` ループの内側で `Dim x As T: x = 式` のようにコロンで宣言と代入をまとめて書いた箇所で「**コンパイルエラーです。：構文エラー**」が発生した。原因を1行単位では特定しきれなかったため、安全策として**全ての `Dim` を各Sub/Functionの先頭にまとめ、コロンでの複合文をやめて宣言と代入を別行に分離**する書き方に統一したところ解消した。以降このプロジェクトのVBAコードは「Dimは手続きの先頭にまとめる」を基本スタイルとする
...
| 2026-07-02 | ページからの文字溢れ対策として、Markdownの表を箇条書きに自動変換する機能・コードブロック省略・H1見出しでのスライド分割・PowerPointオートフィットを追加したが、表の自動変換は「行の意味関係が失われて読みにくい」との判断で撤回。表・文字溢れは手動での内容整形を基本方針とし、マニュアルに明記。コードブロック省略・H1分割・オートフィットは維持 |
| 2026-07-02 | `title` フロントマターが省略された場合、本文の最初の `#` 見出し（なければ最初の `##` 見出し）から自動取得するように変更。`subtitle`/`author`/`date` は元々省略可だったため、これでフロントマター自体を丸ごと省略でき、README.md等の既存Markdownをそのまま渡してスライド生成できるようになった |
| 2026-07-02 | Markdownの表をネイティブなPowerPointの表に変換する機能・本文/表の重なり対策（動的な高さ推定）・コードフェンス対応の見出し検出などを試作したが、レイアウト崩れが解消しきれず方針転換。当時の `make_pptx.py` は `make_pptx_bak.py` として保存し、`make_pptx.py` 自体はタイトル自動取得までの安定版に戻した。以後、Marpのデザインを活かしたスライド作成は [marp_slides](https://github.com/masauehr/marp_slides) プロジェクト（特に `--pptx-editable` オプション）を軸に検討していく方針とし、`ppt_auto` は定型レイアウトでの生成用途として一旦区切りとする |
| 2026-07-02 | VBA版 `MakePPTXFromMarkdown` にも `title` 自動取得機能を追加（`ExtractAutoTitle` 関数）。`make_pptx.py` と同じロジック（最初の `# ` 見出し、なければ最初の `## ` 見出しを使用）で、フロントマターの `title` を省略したMarkdownファイルでもタイトルを自動設定できるようになった |
| 2026-07-02 | `make_pptx.py` に表機能・コードブロック表示機能を再実装（`make_pptx_bak.py` をベースに改修）。Markdownの表（`\| a \| b \|`）は本物のPowerPoint表（`add_table_shape`）に、コードブロック（```` ``` ````）はディレクトリツリー等のファイル構成表示向けに等幅フォント（Courier New）のテキストボックス（`add_code_box`）に、それぞれ変換されるようにした（以前はコードブロックは省略していたが、構成図等の情報が失われるため表示する方針に変更）。本文・表・コードブロック・画像・引用の各要素は生成時に文字量から高さを概算して縦に積み上げる方式（cursorベースのレイアウト）とし、要素同士が重ならないようにした。ただし1スライドに詰め込む内容が極端に多い場合（本文＋表＋コードブロックが同時にある等）はスライド下端をはみ出すことがあり、その場合は既存方針通り手動でのスライド分割・内容整理が必要（`mcp_remort.md` の実例で確認済み） |
| 2026-07-02 | VBA版 `MakePPTXFromMarkdown` にも表機能・コードブロック表示機能を追加し、Python版と機能をほぼ揃えた。`AddTableShape`（`Shapes.AddTable` によるネイティブPowerPoint表）・`AddCodeBox`（等幅フォントのテキストボックス）・`ParseSlideBody`（本文/表/コードブロックへの行単位振り分け）を新規追加し、`MakeContentSlide` はcursor方式で本文・表・コードブロックを縦に積み上げて重なりを防止する方式に変更（Python版と同じ設計）。実機テストで2件のコンパイルエラーを修正: ①`Collection`型引数を（`ByVal`なしの既定のByRefで）宣言していたため`For Each`のVariant変数を渡すと「ByRef 引数の型が一致しません」エラーになり`ByVal`指定で解消、②`If`/`For`ブロック内で`Dim x As T: x = 式`とコロンで宣言・代入をまとめていた箇所で原因不明の「構文エラー」が発生したため、全関数の`Dim`を手続き先頭にまとめコロン複合文を廃止するスタイルに統一して解消。以後この書き方をVBAコードの基本スタイルとする |
