...
| ` ```コードブロック``` ` | スライドには不向きなため本文から除外される |
| `\|表\|の行\|` | **自動変換はしない**。生のパイプ記法のままスライドに出る（下記参照） |
...
これにより、README.mdなど既存のMarkdownファイルをコピーせずそのまま `python3 make_pptx.py README.md` のように渡しても、タイトル付きのスライドが生成できる（ただし表・コードブロックが多いファイルは前述のとおり読みにくくなりがちなので、内容を絞り込んだ専用ファイルを別途作るのが望ましい）。
...
Markdownの表（`| a | b |`）は自動的に箇条書きへ変換しない。一時期、表を箇条書きに機械変換する機能を試したが、行の意味関係が失われて逆に読みにくくなったため撤回した。表がある元文書をスライド化する場合は、`- 項目: 値` のような箇条書きに**手動で**書き直すこと。

同様に、ページからの文字溢れも自動処理（フォント自動縮小以外）には頼らず、**内容を絞り込んで手動調整するのが基本方針**。PowerPointのオートフィット（`autofit`、テキストボックスに収まらない場合にPowerPoint側でフォントサイズを縮小する機能）は保険として有効にしてあるが、これは「読めるサイズを保ったまま収める」ことまでは保証しない。生の技術文書やREADMEを丸ごと貼り付けるのではなく、スライドごとに3〜5個程度の簡潔な箇条書きに要約してから流し込むと、見やすい資料になる。
