...
表（`| ... |`）は本物のPowerPoint表に、コードブロック（```` ``` ````）は等幅フォントのテキストボックス（ディレクトリツリー等のファイル構成表示向け）に、それぞれ自動変換される。VBA版も表・コードブロックに対応しているが、`>` 引用と `![]()` 画像挿入は非対応（詳細は[ppt_auto_manual.md](ppt_auto_manual.md)参照）。
